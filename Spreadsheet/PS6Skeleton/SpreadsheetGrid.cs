...
//
// Slight modifications made by Caden Erickson for PS6
...
public class SpreadsheetGrid : ScrollView, IDrawable
{
    ...
    public void Draw(ICanvas canvas, RectF dirtyRect)
    {
        ...
        for (int x = 0; x < COL_COUNT - _firstColumn; x++)
        {
            canvas.FontColor = (_selectedCol - _firstColumn == x) ? Colors.White : Colors.Black;
            Font f = (_selectedCol - _firstColumn == x) ? Font.DefaultBold : Font.Default;
            DrawColumnLabel(canvas, x, f);
        }
        ...
        for (int y = 0; y < (ROW_COUNT - _firstRow); y++)
        {
            canvas.FontColor = (_selectedRow - _firstRow == y) ? Colors.White : Colors.Black;
            Font f = (_selectedRow - _firstRow == y) ? Font.DefaultBold : Font.Default;
            DrawRowLabel(canvas, y, f);
        }
    ...
    private void DrawColumnLabel(ICanvas canvas, int x, Font f)
    {
        ...
        {
            canvas.FillColor = Color.FromArgb("87D1CE");
            canvas.StrokeColor = Colors.Transparent;
            canvas.FillRectangle(LABEL_COL_WIDTH + x * DATA_COL_WIDTH, 0, DATA_COL_WIDTH, LABEL_ROW_HEIGHT);
        }
        else
        {
    ...
    private void DrawRowLabel(ICanvas canvas, int y, Font f)
    {
        ...
        if (_selectedRow - _firstRow == y)
        {
            canvas.FillColor = Color.FromArgb("87D1CE");
            canvas.StrokeColor = Colors.Transparent;
            canvas.FillRectangle(1, LABEL_ROW_HEIGHT + y * DATA_ROW_HEIGHT, LABEL_ROW_HEIGHT - 1, DATA_ROW_HEIGHT);
        }
        else
        {
            canvas.FillColor = Colors.Transparent;
